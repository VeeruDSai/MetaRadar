  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">Red-Team Contradiction Engine</h1>
      ...
      {loading && (
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-40 rounded-xl bg-slate-100 dark:bg-slate-900/40 animate-pulse border border-slate-200 dark:border-slate-800" />
          ))}
        </div>
      )}
        ...
        <div className="space-y-4">
          {contradictions.map((c) => (
            <div
              ...
              className="rounded-xl border border-red-200 dark:border-red-900/30 bg-white dark:bg-slate-900/60 p-5 space-y-4 shadow-sm"
            >
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-200">{c.description}</h3>
                </div>
              </div>

              {/* Side-by-Side Verbatim Evidence Excerpts */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2">
                <div className="p-3.5 rounded-lg bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 space-y-1.5">
                  <div className="flex items-center justify-between text-[11px] font-semibold text-slate-600 dark:text-slate-400 uppercase">
                    <span>Claim A (Primary Ingestion)</span>
                    <span className="font-mono text-slate-500">ID: {c.claim_a_id}</span>
                  </div>
                  <p className="text-xs text-slate-800 dark:text-slate-300 leading-relaxed select-text font-mono">
                    {c.claim_a_excerpt || 'Evidence excerpt recorded in bronze stream.'}
                  </p>
                </div>

                <div className="p-3.5 rounded-lg bg-red-50/50 dark:bg-slate-950/70 border border-red-200 dark:border-red-900/40 space-y-1.5">
