...
def export_contracts():
    """Dumps the live OpenAPI schema to contracts/openapi.json and re-emits the
    hand-maintained TypeScript contract template to frontend/types/api.ts.

    Honesty note: frontend/types/api.ts is NOT derived from the OpenAPI schema.
    The script re-emits a static template verbatim, so the CI diff gate only
    detects direct edits to the canonical copy — it cannot catch drift between
    the FastAPI schema and the TS interfaces. Until real codegen
    (e.g. openapi-typescript / datamodel-code-generator) is adopted, schema-to-TS
    drift must be caught by human review of contracts/openapi.json diffs.
    """
    ...
    ts_content = """// Hand-maintained TypeScript API contract — kept in sync manually with backend/app/schemas.
