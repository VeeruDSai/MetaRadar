...
class BioPharmaDiveRSSConnector(SourceConnector):
    """BioPharma Dive RSS feed async adapter (REQ-P10-01).

    Tier 3 Discovery source: parses the official BioPharma Dive news feed
    (https://www.biopharmadive.com/feeds/news/) with stdlib xml.etree.
    Items are filtered by domain profile keywords; stores direct article
    links as canonical URLs for truthful provenance.
    """

    source_id = "biopharmadive"
    source_type = "news"
    freshness_class = "delayed"
    DEFAULT_RSS_URL = "https://www.biopharmadive.com/feeds/news/"
    ...
    async def run_profile(
        ...
        try:
            ...
            try:
                ...
                logger.warning("BioPharma Dive profile %s feed unavailable/invalid XML: %s", profile_id, parse_or_fetch_err)
            except Exception as inner_ex:
                logger.warning("BioPharma Dive profile %s unexpected error: %s", profile_id, inner_ex)
            ...
            logger.exception("BioPharmaDiveRSSConnector profile %s failed", profile_id)
            return self._fail(profile_id, started, str(e))
    ...
    def _parse_item(
        ...
        profile_id: str = "haemophilia_biopharmadive",
        ...
        fingerprint = generate_fingerprint(title=title, published_at=published_at, publisher="BioPharma Dive")
    ...
    def _fail(self, profile_id: str, started: datetime, err: str) -> ProfileRunResult:
    ...
    @staticmethod
    def _looks_like_http_url(url: Optional[str]) -> bool:
        if not url:
            return False
        cleaned = url.strip()
    ...
    @staticmethod
    def _parse_rfc2822(raw_date: str, fallback: datetime) -> datetime:
        if not raw_date:
            return fallback
        try:
            ...
            try:
                dt = datetime.strptime(raw_date.strip()[:25], "%a, %d %b %Y %H:%M:%S")
