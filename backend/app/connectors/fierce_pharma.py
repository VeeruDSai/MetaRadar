...
class FiercePharmaRSSConnector(SourceConnector):
    """Fierce Pharma RSS feed async adapter (REQ-P9-07).

    Tier 3 Discovery source: parses the official Fierce Pharma RSS feed
    (https://www.fiercepharma.com/rss/xml) with stdlib xml.etree. Items are
    ...
    async def run_profile(
        ...
        try:
            ...
            try:
                ...
                logger.warning("Fierce Pharma profile %s feed unavailable/invalid XML: %s", profile_id, parse_or_fetch_err)
            except Exception as inner_ex:
                logger.warning("Fierce Pharma profile %s unexpected error: %s", profile_id, inner_ex)
            ...
            logger.exception("FiercePharmaRSSConnector profile %s failed", profile_id)
            return self._fail(profile_id, started, str(e))
    ...
    def _parse_item(
        ...
        fingerprint = generate_fingerprint(title=title, published_at=published_at, publisher="Fierce Pharma")
