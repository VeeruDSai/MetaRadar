...
| NewsAPI | **LIVE** | Core MVP | REST `everything`/`top-headlines` | Free tier modest (documented as ~100–500/day by plan; MUST be quota-aware) | Real industry news + press releases | Cache (24h) → bronze → synthetic; quota-aware batching (one fetch per 2h cadence, incremental since last run) |
...
Every high-priority card renders Q1–Q4 + evidence + **F-I-S badge** (FACT / INTERPRETATION / SPECULATION / INSUFFICIENT), with the completeness strip (Q1✓ Q2✓ Q3✓ Q4✓ ⛓✓ F-I-S✓). Panel tints per UI doc §15.1 (`#F0F4FF / #FFF4E6 / #F0FFF4 / #FFF0F0`).
...
| Weekly digest + function filters | — | G6 | Digest service on brief agent | `services/digest_service.py`, `/digest`, Celery | Digest beat (function tabs) | EV-1 on digest items; function filter works |
...
- **Tasks:** Weekly digest service (7d window → ranked developments → role-filtered variants, export Markdown/PDF); digest UI + Celery nightly generation; watchlist API + `/watch/{entity}` view; notification/alerts badge for confluence/missing/contradiction; email-style digest preview.
...
- [ ] **Working clickable prototype** — Docker Compose full stack (backend, frontend, postgres+pgvector, redis, celery)
...
**Unchanged (deliberately):** the five intelligence mechanisms, the four-question framework, the 10-node LangGraph pipeline, the tech stack (Next.js/FastAPI/LangGraph/PostgreSQL+pgvector/Redis/Celery/local AI), the Docker Compose deployment, the guardrails, the B.Pharm ontology ownership, and the 500-signal synthetic fallback.
