...
| R6 | **Model uncertainty** — local models (Gemma 3 4B, BART) produce weaker reasoning | Small local GPU (4 GB VRAM) + small models; quantization; capability envelope vs commercial LLMs | Confidence scores; F-I-S labels; provider chain + fallback logged and surfaced in UI (model metadata) | Model-agnostic config via `LLMProvider`; known capability envelope documented; provider chain per `LLM_PROVIDER` (Gemma → Grok → BART degraded factual); optional hosted Grok gated by the external-LLM privacy gate (R28) | Human review required for all AI-generated briefs |
| R30 | **Local Gemma cannot fit/initialize on 4 GB VRAM** — model weights (~2.6 GB Q4) + KV cache + context exceed GPU memory; init or inference failure | RTX 3050 4 GB VRAM limit; long context; runner overhead | Provider health (`/api/v1/health/models`); model init/inference errors caught at provider boundary; metadata fallback_from/fallback_reason | NEVER crash: fallback chain Gemma → Grok (if configured & privacy-gated) → BART degraded factual → source-grounded factual signal + human-review flag; weights/KV-cache/context budgeted separately via `LLM_DEVICE`/`LLM_DTYPE`/`MAX_CONTEXT_TOKENS`/`MAX_OUTPUT_TOKENS` (Master Plan §14.1) | Dashboard stays alive; operators see provider/degraded badge and choose whether to enable Grok or reduce context |
...
*Risk Register v1.3 · August 13, 2026 · Novo Nordisk GBS Hackathon 2026 — Problem Statement #3 · Team: Aura Pharmers (MSRIT) · Team Lead: Sanjana Rathore B.*
